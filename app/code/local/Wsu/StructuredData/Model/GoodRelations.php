<?php
class Wsu_StructuredData_Model_GoodRelations extends Mage_Core_Model_Abstract {
    ...
    protected function _construct() {
        $this->_init('structureddata/goodrelations');
        // helper
        $this->div     = Mage::app()->getHelper('structureddata/Div');
        $this->sysinfo = Mage::app()->getHelper('structureddata/Sysinfo');
        // other ontologies
        $this->RDFS    = Mage::getModel('structureddata/RDFs');
        $this->VCARD   = Mage::getModel('structureddata/VCard');
    }
    /**
     * rdf format helper
    ...
    protected function initAddressVCard($URI = NULL) {
        if (!isset($URI))
            $URI = $this->getURI("address");
        if (!isset($this->cacheAddressVCard)) // cachable
            {
            $address                 = $this->Business->getAddress();
            $this->cacheAddressVCard = $this->getEmptyVCard();
            $this->cacheAddressVCard->setStreetaddress(@$address["streetaddress"])->setPostalcode(@$address["postalcode"])->setLocality(@$address["locality"])->setCountryname(@$address["countryname"])->setTel(@$address["tel"])->setEmail(@$address["email"]);
        }
    ...
    protected function initPOSAddressVCard($URI = NULL) {
        if (!$this->Business->hasPOS())
            return; // company has no POS
        if (!isset($URI))
            $URI = $this->getURI("posAddress");
        if (!isset($this->cachePOSAddressVCard)) // cachable
            ...
            $this->cachePOSAddressVCard->setStreetaddress(@$address["streetaddress"])->setPostalcode(@$address["postalcode"])->setLocality(@$address["locality"])->setCountryname(@$address["countryname"])->setTel(@$address["tel"])->setEmail(@$address["email"]);
        }
    ...
    protected function getEmptyVCard() {
        $vCardModel = Mage::getModel('structureddata/VCard');
    ...
    public function businessEntity() {
        ...
        $attributes       = Mage::getResourceModel('eav/entity_attribute_collection')->setEntityTypeFilter($product->getResource()->getTypeId())->addFieldToFilter('attribute_code', 'manufacturer') // This can be changed to any attribute code
            ->load(false);
    ...
    protected function availableAtOrFrom() {
        $settings = Mage::getStoreConfig("wsustructureddata");
    ...
    protected function eligibleRegions() {
        ...
        if (!is_array($countries))
            ...
            foreach ($countries as $countrycode) {
                $properties = array(
                    "gr:eligibleRegions" => $countrycode
                );
...
<div rel=\"gr:hasInventoryLevel\">
	<div typeof=\"gr:QuantitativeValue\">
        ...
        //review stuff
        $statements .= "
		<div rel=\"foaf:maker\" resource=\"urn:jeremybass:StructuredData:v1.2.6.5\"></div>";
    ...
    protected function pReview() {
        ...
        if ($ratings['rating_summary']) {
            if ($ratings['rating_summary'] == 1)
                $rating = 5;
            else
                $rating = round($ratings['rating_summary'] / 20);
    ...
    protected function pHasPriceSpecification() {
        ...
        if ($priceIncludesTax)
            $propArray["gr:valueAddedTaxIncluded"] = 'true';
        else
            $propArray["gr:valueAddedTaxIncluded"] = 'false';
        // check if special price
        if ($this->Product->getSpecialPrice() && strtotime($this->Product->getSpecialToDate()) > strtotime("now")) {
            // Add tax if needed
            if ($priceIncludesTax)
                $price = $tax->getPrice($this->Product, $this->Product->getPrice(1));
            else
            ...
            // Add tax if needed
            if ($priceIncludesTax)
                $price = $tax->getPrice($this->Product, $this->Product->getSpecialPrice(1));
            else
                $price = $this->Product->getSpecialPrice(1);
            ...
            if ($product->getTypeId() == "bundle") {
                ...
                // Add tax if needed
                if ($priceIncludesTax)
                    $price = $tax->getPrice($this->Product, $this->Product->getFinalPrice(1));
                else
                    $price = $this->Product->getFinalPrice(1);
    ...
    protected function pValidFromThrough() {
        ...
        if ($this->Product->getSpecialPrice() && ($specialFrom <= $now) && ($now <= $specialTo)) // for special prices check validity
            ...
            if (!$validThrough)
                $validThrough = date("Y-m-d H:i:s", strtotime("now")); // valid till now
        }
        // 2: product valid through
        elseif (($validThrough = $this->pValidThroughProduct()) != "") {
            if (strtotime($validThrough) < $now)
                $validFrom = $validThrough; // expired
            else
                $validFrom = date("Y-m-d", strtotime("now"));
        }
    ...
    protected function pProduct() {
        ...
        if ($product->getData('color')) {
            if (substr(Mage::getVersion(), 2, 1) > 4)
                $colorstr = "color";
            else
                $colorstr = "Color";
        ...
        foreach ($categoryIds as $categoryId) {
            ...
            if ($category->getName())
                $statements .= $this->rdff->property("gr:category", htmlentities($category_name . $category->getName(), ENT_QUOTES, "UTF-8"), NULL, $this->sysinfo->getLocaleCode());
        }
        $settings = Mage::getStoreConfig("wsustructureddata");
        ...
        if ($strongId) {
            ...
            switch ($strongid_type) {
                case 'gtin14':
                    ...
                    if (strlen($strongId) == 14)
                        $valid = true;
                    break;
                case 'gtin8':
                    $strongid_property = "gr:hasGTIN-8";
                    if (strlen($strongId) == 8)
                        $valid = true;
                    break;
                default: // ean13
                    $strongid_property = "gr:hasEAN_UCC-13";
                    if (strlen($strongId) == 13)
                        $valid = true;
                    break;
            }
            if ($valid)
                $statements .= $this->rdff->propertyAuto($strongid_property, $strongId, $this->getReplacements());
        }
